...
func _physics_process(delta):
	...
	jump = (DEFAULT_CONTROL_JUMP != "" and Input.is_action_pressed(DEFAULT_CONTROL_JUMP) && CONTROL_ENABLE) || simulate_jump
	if walk_left:
		target_speed -= 1
	if walk_right:
		target_speed += 1
	...
	var _cams = get_tree().get_nodes_in_group("CameraCustom")
	if IS_PREVENT_OUTSIDE_SCREEN and not _cams.empty():
		var cam = _cams.back()
		
		parent.global_position.x = clamp(
...
func custom_move_and_slide(custom_velocity, custom_floor_normal) -> Vector2:
	if parent is KinematicBody2D:
		...
		if jump and on_floor:
			...
			vel = parent.move_and_slide_with_snap(custom_velocity, snap_floor_pixel * Engine.time_scale, custom_floor_normal)
...
func jump_start(var check_condition = true) -> void:
	if check_condition:
		if !on_floor:
			...
			if (DEFAULT_CONTROL_JUMP != "" and Input.is_action_pressed(DEFAULT_CONTROL_JUMP)) && CONTROL_ENABLE:
