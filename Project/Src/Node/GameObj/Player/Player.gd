...
export (String) var attack_hotkey = "game_action"
export (String) var jump_hotkey = "game_jump"

#Player's stats
const HP_BASE : int = 28
const MP_BASE : int = 20
const DAMAGE_BASE = 2
const DEFAULT_INVIS_TIME : float = 1.3
...
var proj_ring_boomerang = preload("res://Src/Node/GameObj/PlayerProjectile/MM4_RingBoomerang.tscn")
var dmg_counter = preload("res://Src/Node/GUI/DamageCounter.tscn")
var explosion_effect = preload("res://Src/Node/GameObj/Effects/Explosion/Explosion.tscn")
...
func _on_PlatformerBehavior_fell_into_pit() -> void:
	if Difficulty.difficulty < Difficulty.DIFF_NORMAL:
		...
		if Difficulty.difficulty == Difficulty.DIFF_NEWCOMER:
			current_hp -= 1
			invis_timer.start(DEFAULT_INVIS_TIME)
		if Difficulty.difficulty == Difficulty.DIFF_CASUAL:
			current_hp -= 7
			invis_timer.start(0.6)
...
func start_screen_transition(normalized_direction : Vector2, duration : float, reset_vel_x : bool, reset_vel_y : bool, start_delay : float, transit_distance : float):
	...
	platformer_sprite.animation_paused = true
	palette_ani_player.playback_active = false
...
func is_sliding_key_pressed() -> bool:
	return (
		Input.is_action_just_pressed(jump_hotkey) and Input.is_action_pressed("game_down") or
		Input.is_action_just_pressed("game_slide")
	)
...
#		return
	...
	if curr_weapon_idx == 1:
		global_var.current_player_primary_color = Color("ffe0a8")
...
func teleport_out():
	play_teleport_out_sound()
	$PlatformerSprite/TeleportPlayer.play("TeleportOut")
	platformer_sprite.character_platformer_animation.stop()
